      ...
      <ClipPathMaskSection id="projects" maskType="diagonal" className="py-24 border-t border-[#262626] bg-[#000000]">
        <div className="container mx-auto px-6 max-w-6xl relative z-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start text-left" data-stagger-cards>
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <h4 className="text-sm font-bold font-display uppercase tracking-wide truncate max-w-[200px]">
                          {project.name}
                        </h4>
                      </div>

                      {/* Tech stack badges footer line */}
                      <div className="flex flex-wrap gap-1 items-center mt-2">
                        {project.tags.slice(0, 3).map((tag) => (
                          <span key={tag} className="text-[9px] font-mono bg-[#000000] border border-[#262626] px-1.5 py-0.5 rounded text-[#8A8A8A]">
                            {tag}
                          </span>
                        ))}
                        {project.tags.length > 3 && <span className="text-[8px] font-mono text-[#5C5C5C]">+{project.tags.length - 3}</span>}
                      </div>
            ...
            {/* Code editor pane frame representation */}
            <motion.div 
            ...
            >
              {activeProject ? (
                <div className="w-full rounded-xl border border-[#262626] bg-[#0A0A0A] shadow-none overflow-hidden font-mono">
                  
                  {/* Tab header casing representing VS Code */}
                  <div className="flex items-center justify-between px-4 py-2.5 bg-[#000000] border-b border-[#262626]">
                    <div className="flex items-center gap-2">
                      ...
                      {/* Active editor file tab badge */}
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0A0A0A] border-t border-x border-[#262626] rounded-t-lg text-xs text-white">
                        <FileCode className="w-3.5 h-3.5 text-white" />
                        <span className="text-[11px] font-mono text-white font-medium">{activeProject.filename}</span>
                        <X className="w-3 h-3 text-[#5C5C5C] ml-1.5 cursor-pointer" />
                      </div>
                    </div>

                    {/* IDE Actions */}
                    <div className="flex items-center gap-2.5 text-[11px] text-[#8A8A8A]">
                      <button 
                        onClick={() => setSelectedHologramProject(activeProject)}
                        className="p-1.5 text-white hover:bg-[#161616] bg-[#000000] border border-[#262626] rounded transition-all flex items-center gap-1 cursor-pointer font-bold shrink-0 underline decoration-[#5C5C5C]"
                        title="Launch Interactive 3D Hologram Inspector"
                      >
                        <Layers className="w-3.5 h-3.5 text-white" />
                        <span className="text-[9px] uppercase tracking-wider">Inspect 3D</span>
                      </button>
                      <span className="text-[#262626]">|</span>
                      <button 
                        onClick={() => handleCopyCode(activeProject.codeSnippet)}
                        className="p-1.5 text-white hover:bg-[#161616] bg-[#000000] border border-[#262626] rounded transition-all flex items-center gap-1 cursor-pointer"
                        title="Copy Code Snippet"
                      >
                        {copysuccess ? <Check className="w-3.5 h-3.5 text-white" /> : <Copy className="w-3.5 h-3.5 text-[#8A8A8A]" />}
                        <span className="text-[9px] uppercase tracking-wider">{copysuccess ? "Copied" : "Copy"}</span>
                      </button>
                      <span className="text-[#262626]">|</span>
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-1.5 text-white hover:bg-[#161616] bg-[#000000] border border-[#262626] rounded transition-all flex items-center gap-1 cursor-pointer underline decoration-[#5C5C5C]"
                      >
                        <Github className="w-3.5 h-3.5 text-white" />
                        <span className="text-[9px] uppercase tracking-wider">GitHub</span>
                      </a>
                    </div>
                  </div>

                  {/* Code editor pane frame */}
                  <div className="relative p-5 h-[340px] overflow-auto text-xs leading-relaxed text-white select-all border-b border-[#262626] bg-[#000000]">
                    {/* Line numbers column */}
                    <div className="absolute left-2 top-5 text-right w-6 text-[#5C5C5C] select-none pr-2 border-r border-[#262626]">
                      {activeProject.codeSnippet.split("\n").map((_, i) => (
                        <div key={i}>{i + 1}</div>
                      ))}
                    </div>

                    <pre className="pl-8 text-left font-mono whitespace-pre text-[11px] md:text-xs text-[#FFFFFF]">
                      <code>{activeProject.codeSnippet}</code>
                    </pre>
                  </div>

                  {/* Footer metadata details block */}
                  <div className="p-4 bg-[#0A0A0A] text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 font-mono">
